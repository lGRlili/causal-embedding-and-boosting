...
def get_copa_dict():
    # 　 获取得到3Wpair对应的单词
    with open("data_sort/cause_effect_pair" + ".file", "rb") as f:
        P_A_B_pair = pickle.load(f)
    with open("data_sort/copa_word_lemma.file", "rb") as f:
        copa_word_lemma = pickle.load(f)
    with open("data_sort/copa_lemma_word.file", "rb") as f:
        copa_lemma_word = pickle.load(f)
    with open("data_sort/pair_lemma_list.file", "rb") as f:
    ...
    with open('data_sort/concept_advcl_pair_pos' + '.file', 'wb') as f:
        pickle.dump(advcl_pair_pos_copa, f)
    with open('data_sort/concept_conj_pair_pos' + '.file', 'wb') as f:
        pickle.dump(conj_pair_pos_copa, f)
    with open('data_sort/concept_inter_pair_pos' + '.file', 'wb') as f:
...
def get_copa_dict_sep(pair_kind):
    ...
    with open('data_sort/concept_' + pair_kind + '_pair_pos' + '.file', 'wb') as f:
        pickle.dump(inter_pair_pos_copa, f)
    print('finish')
...
def get_data():
    ...
    cause_effect_pair = defaultdict(int)
    data_list_line = []
    not_cause_data_list_line = []
    stopkey = [w.strip() for w in codecs.open('../stop_word.txt', 'r', encoding='utf-8').readlines()]

    with open('SEMEVAL_cause_effect_pairs.txt', 'r') as f:
        SEMEVAL_cause_effect_pairs = f.readlines()
    with open('SEMEVAL_NON_cause_effect_pairs.txt', 'r') as f:
        SEMEVAL_NON_cause_effect_pairs = f.readlines()

    for pair in SEMEVAL_cause_effect_pairs:
        pair = pair.replace('\n', '')
        pair = pair.replace('\t', ' ')
        cause_word, effect_word = pair.split(' ')
        cause_word = lemminaze(cause_word)
        effect_word = lemminaze(effect_word)
        word_pair = cause_word + '_' + effect_word
        if word_pair not in cause_effect_pair:
            # print(sentence)
            cause_effect_pair[word_pair] = 0
            data_list_line.append([cause_word, effect_word, 1])
        print(pair)

    for pair in SEMEVAL_NON_cause_effect_pairs:
        pair = pair.replace('\n', '')
        pair = pair.replace('\t', ' ')
        cause_word, effect_word = pair.split(' ')
        cause_word = lemminaze(cause_word)
        ...
        if word_pair not in cause_effect_pair:
            # print(sentence)
            cause_effect_pair[word_pair] = 0
            not_cause_data_list_line.append([cause_word, effect_word, 0])
        print(pair)

    sample_not_cause_data_list_line = random.sample(not_cause_data_list_line, len(data_list_line))
    print(len(cause_effect_pair), len(data_list_line), len(not_cause_data_list_line), len(sample_not_cause_data_list_line))

    for word_pair in not_cause_data_list_line:
        if word_pair in data_list_line:
            print(word_pair)
    ...
    with open("data_sort/P_word_lemma_num.file", "wb") as f:
        pickle.dump(P_word_lemma_num, f)
    with open("data_sort/copa_word_lemma.file", "wb") as f:
        pickle.dump(copa_word_lemma, f)
    with open("data_sort/copa_lemma_word.file", "wb") as f:
        pickle.dump(copa_lemma_word, f)
    with open("data_sort/pair_lemma_list.file", "wb") as f:
        pickle.dump(pair_lemma_list, f)
    with open("data_sort/cause_effect_pair.file", "wb") as f:
        pickle.dump(cause_effect_pair, f)
    np.save("data_sort/data_list_line" + ".npy", data_list_line)
    np.save("data_sort/not_cause_data_list_line" + ".npy", not_cause_data_list_line)
    np.save("data_sort/sample_not_cause_data_list_line" + ".npy", sample_not_cause_data_list_line)


if __name__ == '__main__':
    # file = open('senForDemo.txt')
    # senForDemo = file.readlines()
    # file = open('train.txt')
    ...
    # 1461    1375
    # 1668 834----总共提取处834对
    # 此时1668中只有821个pair队有覆盖
    # 在原始语料中,共有954个pair,选择了其中的894对
