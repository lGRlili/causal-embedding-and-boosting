        ...
        if len(clause) > 1:
            ...
            for i in range(1, len(clause)):
                ...
                if sen[ziju[0] - sen[0].id].dep_ == 'advcl':
                    ...
                    len_clause_pair = len(congju_a) * len(congju_b) * count_num_advcl
        ...
        if len(clause) > 1:
            ...
            for i in range(0, len(dist_clause)):
                for j in range(i + 1, len(dist_clause)):
                    ...
                    if dist_clause[i][0] > dist_clause[j][0]:
    ...
    if len(last_setence) > 0:
        ...
        for sens_last in last_congju:
            ...
            while last_setence[last_id - last_setence[0].id].dep_ != 'ROOT':
                if (last_setence[last_id - last_setence[0].id].pos_ == 'VERB') & (last_setence[last_id - last_setence[0].id].dep_ == 'advcl'):
            ...
            for sens_now in total_congju:
                ...
                while sen[now_id - sen[0].id].dep_ != 'ROOT':
                    if (sen[now_id - sen[0].id].pos_ == 'VERB') & (sen[now_id - sen[0].id].dep_ == 'advcl'):
...
def cal_intra_advcl_wei_ab_up(sen, total_congju, advcl_p_a_pos_wei, advcl_p_b_pos_wei, str_id_word_count, stopkey):
    # 句内非从句---句内全连接
    # 此处保证,都是原因从句在前,结果从句在后
    # 从理论上来说,同一个句子,只可能有一个结果状语从句,所以在计算的时候,若有多个结果状语从句,需要除掉权重
    for clause in total_congju:
        if len(clause) > 1:
            ...
            for i in range(1, len(clause)):
                ziju = clause[i]
                if sen[ziju[0] - sen[0].id].dep_ == 'advcl':
                    # 注意,len(congju_b)不等于len_stop_a,这是两种选择的策略
                    ...
                    len_clause_pair = len_stop_a * len_stop_b * count_num_advcl
                    if zhuju[0] > ziju[0]:
                        congju_a, congju_b = congju_b, congju_a
                    if len_clause_pair > 0:
                        for wordA in congju_a:
...
def cal_intra_conj_wei_ab_up(sen, total_congju, conj_p_a_pos_wei, conj_p_b_pos_wei, str_id_word_count, stopkey):
    ...
    for clause in total_congju:
        ...
        if len(clause) > 1:
            ...
            for i in range(0, len(dist_clause)):
                for j in range(i + 1, len(dist_clause)):
                    congju_a, len_stop_a = get_str_id_stop(sen, dist_clause[i], str_id_word_count, stopkey)
                    congju_b, len_stop_b = get_str_id_stop(sen, dist_clause[j], str_id_word_count, stopkey)
...
def cal_p_a_b_inter_num_weight_dis_ab_up(sen, total_congju, inter_level_p_a_pos, inter_level_p_b_pos,
...
def cal_p_a_b_inter_num_weight_dis_ab_weight_up(sen, total_congju, inter_level_p_a_pos, inter_level_p_b_pos,
                                      last_setence, last_congju, str_id_word_count, stopkey):
    sen_now, sen_last = [], []
    now_len = len(total_congju)
    last_len = len(last_congju)
    ...
    if len(last_setence) > 0:
            ...
            for sens_now in total_congju:
                ...
                while sen[now_id - sen[0].id].dep_ != 'ROOT':
                    if (sen[now_id - sen[0].id].pos_ == 'VERB') & (sen[now_id - sen[0].id].dep_ == 'advcl'):
                        count_num_now = count_num_now + 1
                    now_id = sen[now_id - sen[0].id].head
                    weight_count = weight_count + 1/(count_num_last * count_num_now)
        ...
        for sens_last in last_congju:
            ...
            for sens_now in total_congju:
                ...
                clause_lasts, len_stop_b = get_str_id_stop(last_setence, sens_last, str_id_word_count, stopkey)
                len_clause_pair = len_stop_a * len_stop_b
                len_temp_level = count_num_last * count_num_now * len_clause_pair * weight_count
                if len_temp_level > 0:
                    for word_last in clause_lasts:
                        inter_level_p_a_pos[word_last] += round(len(clause_now) / len_temp_level, 6)
                    for word_now in clause_now:
...
def cal_intra_advcl_wei_up(sen, total_congju, advcl_p_a_b_pos_wei, str_id_word_count, count_advcl, stopkey):
    # 句内非从句---句内全连接
    for clause in total_congju:
        if len(clause) > 1:
            zhuju = clause[0]
...
def cal_intra_conj_wei_up(sen, total_congju, conj_p_a_b_pos_wei, str_id_word_count, count_conj, stopkey):
...
def cal_p_a_b_inter_num_weight_dis_up(sen, total_congju, inter_level_p_a_b_pos, str_id_word_count, last_setence, last_congju, count_inter, stopkey):
    sen_now, sen_last = [], []
    now_len = len(total_congju)
    last_len = len(last_congju)
    clause__len = now_len * last_len
...
def cal_p_a_b_inter_num_weight_dis_weight_up(sen, total_congju, inter_level_p_a_b_pos, str_id_word_count, last_setence, last_congju, count_inter, stopkey):
    sen_now, sen_last = [], []
    now_len = len(total_congju)
    last_len = len(last_congju)
    clause__len = now_len * last_len
